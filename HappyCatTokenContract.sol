...
contract HappyCatToken is IERC20, Ownable {
    using SafeERC20 for IERC20;

    // IUniswapV2Caller public constant uniswapV2Caller =
    //     IUniswapV2Caller(0x617715A9Bf6dD62D1Beb70F29914Fcf821933B39); //pancakeswap v2 caller
    string private  _name = "Happy Cat Token";
    string private  _symbol = "HCT";
    uint8  private  _decimals = 18;          

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tFeeTotal;
    uint256 private _tTotal = 88* 10**9 * 10**18; // 88 Billion 
    ...
    //buyback wallet details
    address public orionWallet = 0x5A90D646b66Ad0E5Ac98E3e2766c4Bb3E756EdbA;
    bool public isMarketingFeeBaseToken = true;
    ...
    //Token Holder Reward Fee
    uint16 public sellRewardFee= 20;
    uint16 public buyRewardFee= 20;
    
    //Liquidity Fee if any
    ...
    uint256 public minAmountToTakeFee = _tTotal/(10000);
    uint256 public maxWallet =_tTotal;
    uint256 public maxTransactionAmount = _tTotal;
    ...
    address private  baseTokenForPair = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //WBNB
    address private  dexRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // pcs v2 router
    ...
    constructor()  {
        ...
        //exclude certain addresses from fee, rewards, Max Tx
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(
            _excluded.length + 1 <= 100,
            "Cannot exclude more than 100 accounts.  Include a previously excluded address."
        );
    ...
    function updateLiquidityFee(
        ...
        require(
            _sellLiquidityFee+(sellMarketingFee)+(sellRewardFee) <= 100,
            "Total Sell fees can not be more than 10%"
        );
        require(
            _buyLiquidityFee+(buyMarketingFee)+(buyRewardFee) <= 100,
            "Total Buy fees can not be more than 10%"
        );
        emit UpdateLiquidityFee(
            _sellLiquidityFee,
            _buyLiquidityFee,
    ...
    function updateMarketingFee(
        ...
        require(
            _sellMarketingFee+(sellLiquidityFee)+(sellRewardFee) <= 100,
            "Total Sell fees can not be more than 10%"
        );
        require(
            _buyMarketingFee+(buyLiquidityFee)+(buyRewardFee) <= 100,
            "Total Buy fees can not be more than 10%"
        );
    ...
    function updateRewardFee(
        ...
        require(
            _sellRewardFee+(sellLiquidityFee)+(sellMarketingFee) <= 100,
            "Total Sell fees can not be more than 10%"
        );
        require(
            _buyRewardFee+(buyLiquidityFee)+(buyMarketingFee) <= 100,
            "Total Buy fees can not be more than 10%"
        );
    ...
    function updateOrionWallet(
        address _orionWallet,
        bool _isMarketingFeeBaseToken
    ) external onlyOwner {
        require(_orionWallet != address(0), "Orion wallet can't be 0");              
        //emit event
        emit UpdateOrionWallet(_orionWallet, _isMarketingFeeBaseToken,orionWallet, isMarketingFeeBaseToken);
        orionWallet = _orionWallet;
        isMarketingFeeBaseToken = _isMarketingFeeBaseToken;
        isExcludedFromFee[_orionWallet] = true;        
        isExcludedFromMaxTransactionAmount[_orionWallet] = true;
    }
    ...
    function excludeFromFee(address account, bool isEx) external onlyOwner {
        require(isExcludedFromFee[account] != isEx, "Account already excluded from Fee");
    ...
    function excludeFromMaxTransactionAmount(address account, bool isEx)
        ...
        onlyOwner
    {
        require(isExcludedFromMaxTransactionAmount[account]!=isEx, "Account already excluded from Max Tx");
    ...
    function takeFee() private lockTheSwap {
        ...
        if (isMarketingFeeBaseToken) {
            ...
            if(baseTokenForMarketing>0){
                if(baseTokenForPair==mainRouter.WETH()){
                    (bool success, )=address(orionWallet).call{value: baseTokenForMarketing}("");
                    if(success){
                        emit MarketingFeeTaken(0, baseTokenForMarketing);
                    }
                }else{
                    IERC20(baseTokenForPair).safeTransfer(orionWallet, baseTokenForMarketing);
                    emit MarketingFeeTaken(0, baseTokenForMarketing);
                }       
            }            
        } else {
            ...
            if(_marketingFeeTokens>0){
                _transfer(address(this), orionWallet, _marketingFeeTokens);
    ...
    function rescueETH() external onlyOwner {
        require(address(this).balance > 0, "Not enough Native Tokens at the moment");
